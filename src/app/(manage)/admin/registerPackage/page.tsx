...
const RegisterPackagePage = () => {
  ...
  async function onSubmit(values: z.infer<typeof registerPackageRule>) {
    if (!selectPk) {
      toast.warning("Vui lòng chọn gói tập trước");
      return;
    }
    setIsLoading(true);
    const dataRegisterTracking: typeRegisterTracking = {
      ...
      totalPrice: calculatePrice(selectPk?.packages?.price, 0),
      isPaid: true,
          ...
          {selectPk ? (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                Giảm: (4 🏅)
                <strong>
                  -{renderVND(calculatePrice(selectPk?.packages?.price, 100))}
                </strong>
              </div>
              <hr />
              <div>
                ...
                <strong className="text-bold text-Primary text-2xl">
                  {renderVND(calculatePrice(selectPk?.packages?.price, 0))}
                </strong>
              </div>
              <ButtonCustom
                variant="custom"
            ...
            <div>Chọn gói tập để xem giá</div>
          )}
        </div>
      </div>
    </div>
